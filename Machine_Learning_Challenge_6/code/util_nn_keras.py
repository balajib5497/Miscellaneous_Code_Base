...
def one_hot_encoding(y, is_train=True):
    global enc
...
#    x[columns_to_scale] = get_scaled_data(x[columns_to_scale])
    x = get_scaled_data(x)
...
#    test_data[columns_to_scale] = get_scaled_data(test_data[columns_to_scale], is_trainig=False)
    
    test_data = get_scaled_data(test_data, is_training=False)
